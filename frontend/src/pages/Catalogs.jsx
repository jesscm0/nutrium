...
function Catalogs() {
  ...
  const fetchCatalog = useCallback((loc = location, nm = name) => {
    ...
    fetch(`http://localhost:3000/api/v1/catalogs?${params.toString()}`)
      .then(res => res.json())
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[200px_1fr_1fr_1fr] gap-4">
              ...
              <div className="flex flex-col gap-4 p-4">
                <div className="flex flex-row items-center bg-nutrium-green-darker/30 w-fit rounded-full px-2 gap-2">
                  ...
                  <p className="uppercase text-sm text-center font-semibold text-nutrium-green-darker">FOLLOW-UP</p>
                </div>
                <p className="text-2xl font-semibold text-nutrium-green-darker">{item.nutritionist?.first_name} {item.nutritionist?.last_name}</p>
                <div className="flex flex-row gap-1">
                  <p className="text-l text-slate-600">{item.service?.description}</p>
                  &bull;
                  <p className="text-l text-slate-600">{item.nutritionist?.professional_id}</p>
                </div>
              </div>
              {/* Grid 3*/}
              ...
              <div className=" p-4">
                <div className="flex flex-col gap-3">
                  <button
                    onClick={() => { console.log(1); setSelectedCatalog(item); setOpenModal(true) }}
              ...
              <div className=" p-4">
                <div className="flex columns-2 my-5 ml-5">
                  ...
                  <div className=" flex flex-col gap-4 mx-5 align-text-top">
                    ...
                    <p className="text-s text-slate-600">{item?.address}</p>
                    <p className="text-s text-slate-600">{item?.district?.name}</p>
                  </div>
                </div>
              </div>
              {/* Grid 7 */}
              <div className=" p-4">
                <div className="flex flex-col my-5 ml-5 gap-4">
                  ...
                  <div className="flex flex-row  ml-30 items-center gap-4">
                    ...
                    <p className="text-s text-slate-600">{item?.price}€</p>
                  </div>
                </div>
              </div>
              {/* Grid 8 */}
  ...
  return (
    <div className="min-h-screen bg-nutrium-light-grey">
      ...
      <div className="mx-auto px-4 py-12 gap-3 flex items-center bg-nutrium-green-darker">
        <input className="ml-9 placeholder:italic placeholder:text-slate-300 block bg-white w-full border border-slate-300 rounded-md py-3 pl-9 pr-3 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-md"
          placeholder={t("placeholderNameService")}
          type="text"
          name="search"
          value={name}
          onChange={(e) => { console.log(e.target.value); setName(e.target.value) }} />
        <input className="placeholder:italic placeholder:text-slate-300 block bg-white w-full border border-slate-300 rounded-md py-3 pl-9 pr-3 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-md"
          placeholder={t("placeholderLocation")}
          type="text"
          name="location"
          onChange={(e) => { console.log(e.target.value); setLocation(e.target.value) }} />
        <button
          className="mr-9 rounded-none bg-nutrium-orange text-white py-3 px-6"
          onClick={() => fetchCatalog(location, name)}
        >{t("search")} </button>
      </div>
      ...
      <AppointmentModal open={openModal} setOpen={setOpenModal} catalog={selectedCatalog} />
    </div>
  )
